...
| `created_at` | string | 事件创建时间，UTC RFC3339Nano。              |
| `expires_at` | string | 事件过期时间，UTC RFC3339Nano；过期事件不应继续下发。 |
...
```json
{
  ...
  "result": "命令输出结果",
  "exit_code": 0, // 进程退出码
  "finished_at": "完成时间，必须为带时区的 RFC3339 格式"
}
```
...
```json
{
  ...
  "finished_at": "2026-07-17T01:30:00.123456789Z"
}
```
